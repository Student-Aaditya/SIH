...
export default function AlumniForm({ show, onClose, onSave, initialData }) {
  ...
  const [skillsInput, setSkillsInput] = useState(""); // controlled string input
    ...
    <div style={overlayStyle}>
      <div style={modalStyle}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10}}>
          <h5>{form.id ? "Edit Alumni" : "Add New Alumni"}</h5>
          <div>
        ...
        <div className="row g-3">
          ...
          <div className="col-md-3">
            ...
            <input type="number" className="form-control" value={form.graduationYear} onChange={e=>handleChange("graduationYear", Number(e.target.value))} />
          </div>
          ...
          <div className="col-md-4">
            ...
            <input className="form-control" value={form.company} onChange={e=>handleChange("company", e.target.value)} />
          </div>

          <div className="col-md-4">
            <label className="form-label">Position / Designation</label>
            <input className="form-control" value={form.position} onChange={e=>handleChange("position", e.target.value)} />
          </div>

          <div className="col-md-6">
            <label className="form-label">Skills (comma separated)</label>
            <input className="form-control" value={skillsInput} onChange={e=>setSkillsInput(e.target.value)} />
          </div>
          ...
          <div className="col-md-6">
            <label className="form-label">Profile Avatar (optional)</label>
          ...
          <div className="col-12">
            <label className="form-label">Certificates (PDF / images)</label>
            <input ref={fileRef} type="file" accept="application/pdf,image/*" multiple className="form-control" onChange={handleFiles} />
            <div style={{marginTop:8}}>
              {(form.certificates||[]).length === 0 ? <div className="text-muted">No certificates added</div> :
                form.certificates.map(c => (
                  <div key={c.id} className="card mb-2 p-2">
                    <div style={{display:'flex', gap:12, alignItems:'center'}}>
                      ...
                      <div style={{flex:1}}>
                        <input className="form-control form-control-sm" value={c.title} onChange={e=>updateCertField(c.id,"title",e.target.value)} />
